...
const ModifCategorie = () => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("modifCategorie?idCategorie=" + idCategorie + "&titre=" + titre);
      ...
      if (response.status === 200) {
        ...
        window.location.href = "/modifCategorie";
      };
        ...
        {demandes.allCategorie.map((zoom) => (
            ...
            <li className="mt-5" key={zoom.idCategorie}>
              <Accordion collapseAll>
                  ...
                  <button onClick={() => openEditModal(zoom.idCategorie)}>Modifier</button>
                </div>
