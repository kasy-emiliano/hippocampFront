...
const StatFo = () => {
    ...
    useEffect(() => {
        ...
        axios.get("/StatFo")
            .then((response) => {
                setDemandes(response.data.stat);
                setTailleDuTableau(response.data.total);
            })
            .catch((error) => {
        ...
        <>
            ...
            <div className="flex items-center justify-start h-full mb-6 ml-16">
                <div className="bg-white rounded-lg shadow-lg p-6 flex items-center space-x-4">
                    ...
                    <div>
                        <h1 className="text-xl font-bold">Nombre total de formation</h1>
                            ...
                            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                ...
                                {demandes.map(demande => (
                                    <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        <td className="px-4 py-3">{demande.an}</td>
                                        <td className="px-4 py-3">{demande.totalnombre}</td>
                                        <td className="px-4 py-3"><Link className="font-medium text-cyan-600 hover:underline dark:text-cyan-500" to={`/StatFom?Annee=${demande.an}`}>
                                            <p>Voir</p>
                                        </Link></td>
